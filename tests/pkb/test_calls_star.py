...
def test_pkb_calls_star_name_name():
    ...
    assert result == ["test1", "test2", "test3", "test4", "test5", "test6", "test7"]


def test_pkb_calls_star_name_name_1():
    queries = parse_query(
        """procedure p, q; Select p such that Calls* ("test1", "test6")"""
    )
    result = evaluate_query(tree, queries[0])
    assert result == []
